...
class _af_loss:
  ...
  def _loss_partial(self, inputs, outputs, aux):
    ...
    def sub(x, axis=0):
      return jax.tree.map(lambda y:jnp.take(y,pos,axis),x)
...
def _get_pw_loss(true, pred, loss_fn, weights=None, copies=1, return_mtx=False):
  ...
  if copies > 1:
    ...
    # intra (L,L,F)
    intra = jax.tree.map(lambda x:x[:L,:L], F)
    mtx, loss = loss_fn(**intra)

    # inter (C*L,L,F)
    inter = jax.tree.map(lambda x:x[L:,:L], F)
    if C == 0:
      i_mtx, i_loss = loss_fn(**inter)

    else:
      # (C,L,L,F)
      inter = jax.tree.map(lambda x:x.reshape(C,L,L,-1), inter)
