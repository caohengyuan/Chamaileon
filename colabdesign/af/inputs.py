...
class _af_inputs:
  ...
  def _get_seq(self, inputs, aux, key=None):
    ...
    if self.protocol == "binder":
      ...
      seq = jax.tree.map(lambda x:jnp.concatenate([seq_target,x],1), seq)
      
    if self.protocol in ["fixbb","hallucination","partial"] and self._args["copies"] > 1:
      seq = jax.tree.map(lambda x:expand_copies(x, self._args["copies"], self._args["block_diag"]), seq)
...
def update_aatype(aatype, inputs):
  ...
  inputs.update(jax.tree.map(lambda x:jnp.where(mask,jnp.asarray(x)[aatype],0),a))
