...
class mk_mpnn_model():
  def __init__(self, model_name="v_48_020",
    ...
    self._model.params = jax.tree.map(np.array, checkpoint['model_state_dict'])
    self._setup()
    self.set_seed(seed)
  ...
  def sample(self, num=1, batch=1, temperature=0.1, rescore=False, **kwargs):
    ...
    return jax.tree.map(lambda *x:np.concatenate(x,0),*O)    
  ...
  def score(self, seq=None, **kwargs):
    ...
    I.update(kwargs)
    key = I.pop("key",self.key())
    O = jax.tree.map(np.array, self._score(**I, key=key))
    O.update(self._get_score(I,O))
    return O
