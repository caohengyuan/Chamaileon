...
class mk_tr_model(design_model):
  ...
  def _get_model(self):
    ...
    def _get_loss(inputs, outputs):
      ...
      log_p = jax.tree.map(jax.nn.log_softmax, outputs)

      # bkg loss
      if self.protocol in ["hallucination","partial"]:
        p = jax.tree.map(jax.nn.softmax, outputs)
        log_q = jax.tree.map(jax.nn.log_softmax, inputs["6D_bkg"])
        aux["losses"]["bkg"] = {}
      ...
      if self.protocol in ["fixbb","partial"]:
        if "pos" in opt:
          pos = opt["pos"]
          log_p = jax.tree.map(lambda x:x[:,pos][pos,:], log_p)
      ...
      # weighted loss
      w = opt["weights"]
      tree_multi = lambda x,y: jax.tree.map(lambda a,b:a*b, x,y)
  ...
  def prep_inputs(self, pdb_filename=None, chain=None, length=None,
    ...
    if self.protocol in ["fixbb", "partial"]:
      ...
      if self.protocol == "partial" and pos is not None:
        ...
        self._inputs["batch"] = jax.tree.map(lambda x:x[p], self._inputs["batch"])
        self.opt["pos"] = p
    ...
    if self.protocol in ["hallucination", "partial"]:
      ...
      self._inputs["6D_bkg"] = jax.tree.map(lambda *x:np.stack(x).mean(0), *self._inputs["6D_bkg"])
  ...
  def run(self, backprop=True):
    ...
    for n in model_num:
      ...
      if backprop:
        ...
        grad = jax.tree.map(np.zeros_like, self._params)
      aux.update({"loss":loss, "grad":grad})
      aux_all.append(aux)
    
    # average results
    self.aux = jax.tree.map(lambda *x:np.stack(x).mean(0), *aux_all)
  ...
  def step(self, backprop=True, callback=None, save_best=True, verbose=1):
    ...
    self._params = jax.tree.map(lambda x,g:x-lr*g, self._params, self.aux["grad"])
  ...
  def get_loss(self, k=None, get_best=True):
    ...
    weighted_losses = jax.tree.map(lambda l,w:l*w, losses, weights)
