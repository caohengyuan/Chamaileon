...
class TargetBinderData(LightningDataModule):
    ...
    def train_dataloader(self, rank=None, num_replicas=None):
        ...
        batch_sampler = TargetBinderLengthBatcher(
            sampler_cfg=self.sampler_cfg,
            metadata_json=self._train_dataset.json,
            rank=rank,
...
class TargetBinderLengthBatcher:
    ...
    def __init__(
        ...
        # Use the target-binder metadata ordering directly for batching.
        num_batches = len(self._data_json)
    ...
    def _sample_indices(self):
        return [item['index'] for item in self._data_json]
    ...
    def _replica_epoch_batches(self):
        ...
        if len(self._data_json) > self.num_replicas:
            replica_json = [self._data_json[i] for i in indices[self.rank::self.num_replicas]]
        else:
            replica_json = self._data_json

        # Each batch contains multiple proteins of the same length.
        sample_order = []
        # Group replica_json (a list of dicts) by 'aligned_binder.seq_len'

        groups = defaultdict(list)
        for item in replica_json:
            # TODO: here we should use modeled sequence length instead of
            # the original sequence length.
            seq_len = item['chain_1']['modeled_seq_len'] + item['chain_2']['modeled_seq_len']
            groups[seq_len].append(item)
        ...
        for seq_len, items in groups.items():
            ...
            num_batches = math.ceil(len(items) / max_batch_size)
            for i in range(num_batches):
                batch_items = items[i*max_batch_size:(i+1)*max_batch_size]
                batch_indices = [x['index'] for x in batch_items]
        ...
        if self.shuffle:
            ...
            # print(f'RANK: {self.rank}, sample_order: {sample_order}')
    ...
    def _create_batches(self):
        ...
        while len(all_batches) < self._num_batches:
            ...
            num_augments += 1
            # whether to remove the following code?
    ...
    def __iter__(self):
        ...
        self.epoch += 1
        # print(f'RANK: {self.rank}, sample_order: {self.sample_order}')
