...
from multiflow.data.target_binder_eval_datasets import PDBTargetBinderEvalDataset
...
class EvalRunner:
    ...
    def __init__(self, cfg: DictConfig):
        ...
        # Read in checkpoint.
        if cfg.inference.task == 'target_binder_eval':
            ckpt_path = cfg.inference.target_binder_eval_ckpt_path
        else:
    ...
    def run_sampling(self):
        ...
        if self._infer_cfg.task == 'target_binder_eval':
            eval_dataset, _ = eu.dataset_creation(
                PDBTargetBinderEvalDataset, self._infer_cfg.pdb_target_binder_eval_dataset, self._infer_cfg.task
            )
        else:
...
@hydra.main(version_base=None, config_path="../configs", config_name="inference_target_binder_eval")
def run(cfg: DictConfig) -> None:
    ...
    def compute_metrics():
        if cfg.inference.task == 'target_binder_eval':
            log.info("Target binder eval metrics are handled separately after inference outputs are generated.")
        else:
            raise ValueError(f'Unknown task {cfg.inference.task}')
