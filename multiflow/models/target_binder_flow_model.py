...
from multiflow.models.target_binder_node_feature_net import TargetBinderNodeFeatureNet
...
class TargetBinderFlowModel(nn.Module):

    def __init__(self, model_conf):
        super(TargetBinderFlowModel, self).__init__()
        self._model_conf = model_conf
        self._ipa_conf = model_conf.ipa
        self.rigids_ang_to_nm = lambda x: x.apply_trans_fn(lambda x: x * du.ANG_TO_NM_SCALE)
        self.rigids_nm_to_ang = lambda x: x.apply_trans_fn(lambda x: x * du.NM_TO_ANG_SCALE) 
        self.node_feature_net = TargetBinderNodeFeatureNet(model_conf.node_features)
        self.edge_feature_net = EdgeFeatureNet(model_conf.edge_features)

        # predict logits for amino acid types
    ...
    def forward(self, input_feats):
        ...
        binder_so3_t = input_feats['binder_so3_t']
        binder_r3_t = input_feats['binder_r3_t']
        binder_cat_t = input_feats['binder_cat_t']
        target_so3_t = input_feats['target_so3_t']
        target_r3_t = input_feats['target_r3_t']
        target_cat_t = input_feats['target_cat_t']
        trans_t = input_feats['trans_t']
        rotmats_t = input_feats['rotmats_t']
        aatypes_t = input_feats['aatypes_t'].long()
        trans_sc = input_feats['trans_sc']
        aatypes_sc = input_feats['aatypes_sc']
        target_seq_len = input_feats['target_seq_len']
        binder_seq_len = input_feats['binder_seq_len']
        hotspot = input_feats['hotspot']

        # Initialize node and edge embeddings
        init_node_embed = self.node_feature_net(
            binder_so3_t=binder_so3_t,
            binder_r3_t=binder_r3_t,
            binder_cat_t=binder_cat_t,
            target_so3_t=target_so3_t,
            target_r3_t=target_r3_t,
            target_cat_t=target_cat_t,
            res_mask=node_mask,
