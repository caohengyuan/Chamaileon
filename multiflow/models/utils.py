...
def calc_aatype_metrics_list_of_arrays(generated_aatypes_list):
    all_aatypes = np.concatenate([arr.flatten() for arr in generated_aatypes_list])
    unique_aatypes, raw_counts = np.unique(all_aatypes, return_counts=True)

    clean_counts = np.zeros(20)
    for i in range(20):
        if i in unique_aatypes:
            clean_counts[i] = raw_counts[np.where(unique_aatypes == i)[0][0]]
